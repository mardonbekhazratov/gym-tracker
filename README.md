...
| Native wrap | @capacitor/core + @capacitor/android + @capacitor/app |
...
### Android back button (native)

On the Android APK the hardware / gesture back button stays inside the app:

- An open sheet or dialog closes first.
- On a session detail, it returns to the History list.
- On the History / Progress / Settings tab, it returns to the Today tab.
- On the Today tab, it exits the app.

The service worker is registered on **web only** — never inside the Capacitor
WebView — so a freshly installed APK always runs its own bundled build instead
of a stale precache.
...
│  └─ test-navigation.mjs     # one-off test for back-button route decisions
...
└─ src/
   ...
   │  ├─ navigation.ts        # hardware back-button route decisions
   │  ├─ overlayStack.ts      # sheet/dialog stack so back dismisses overlays
   │  └─ useAndroidBackButton.ts  # wires the native back button to the router
   ├─ components/
   │  ├─ BottomNav.tsx
